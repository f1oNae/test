...
#include "vtkSlicertestModuleMRMLExport.h"

/// \brief MRML node to represent a 3D view.
///
/// View node contains view parameters.
class VTK_SLICER_TEST_MODULE_MRML_EXPORT vtkMRMLtestViewNode
    : public vtkMRMLViewNode
{
public:

    static vtkMRMLtestViewNode* New();
    vtkTypeMacro(vtkMRMLtestViewNode, vtkMRMLViewNode);
    ...
    vtkSetClampMacro(RenderingMode, int, 0, vtkMRMLtestViewNode::RenderingMode_Last - 1);
...
protected:
    ...
    vtkMRMLtestViewNode();
    ~vtkMRMLtestViewNode();
    vtkMRMLtestViewNode(const vtkMRMLtestViewNode&);
    void operator=(const vtkMRMLtestViewNode&);
